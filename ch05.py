...
def main():
    ...
    st.title("라이브러리 선택")
    iris = load_data()
    df = st.data_editor(iris) # 권장 / 내일 오후 시간에도 적용해라.
    ...
    cols = st.multiselect("복수의 컬럼을 선택해라", iris.columns)

    result1 = iris.loc[:,  cols].reset_index(drop=True)
    st.data_editor(result1)
